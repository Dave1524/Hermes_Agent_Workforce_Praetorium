...
| Working memory | `~/.hermes/profiles/research_analyst/memories/MEMORY.md` | runtime state — NOT backed up; regenerated by agent runs, consolidated nightly (NUC-21) |
...
The `research_analyst` profile reaches three services, all via warm localhost transports:
...
'FETCH BLOCKED: <reason>' and invent nothing." -p research_analyst` — a real page body proves fetch;
...
research_analyst` — had Brave return `https://www.ecb.europa.eu/` and local headless Chrome render the
