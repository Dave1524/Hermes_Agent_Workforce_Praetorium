...
echo "--- m1-signal-scan: PINNED FAIL-OPEN — a missing task file does NOT refuse ---"
# Characterization, not approval. `exec claude -p "$(cat "$TASK_FILE")"` puts the failing
# `cat` inside a command substitution used as an ARGUMENT, and `set -e` does not propagate
# that: the substitution yields the empty string and the agent is launched with an EMPTY
# PROMPT, exit 0. Verified 2026-09-03 against bin/run_m1_signal_scan_cc.sh:19.
#
# It is a two-layer fail-open, and the second layer is why it is silent. This job sets no
# AGENT_VERIFY_CMD, so bin/agent_propose.sh:331 skips the artifact check entirely and an
# empty-prompt run lands on line 440 as `OK: run completed, agent produced no proposal` —
# NOPROPOSAL, exit 0, indistinguishable from a real "nothing to report". That is verbatim the
# class the same file's line 278 comment says cost 20 consecutive runs. Its three research
# siblings are covered because bin/proposal_or_decline.sh is wired as their AGENT_VERIFY_CMD;
# m1 was never given one. Tracked as W15 in design/open-decisions.md.
#
# WHEN THIS IS FIXED THESE THREE GO RED. That is the intent — read the row, then rewrite them
# to the refusal shape used by the no_inbox group above.
home=$(make_m1_home no_task)
rc=$(run_m1 "$home")
assert 'W15: exits 0 despite cat(1) failing (set -e does not see a substitution)' "[ '$rc' = 0 ]"
assert 'W15: the agent IS launched, with an empty prompt' \
  "[ -f '$home/claude_argv.log' ] && [ \"\$(sed -n 2p '$home/claude_argv.log')\" = '' ]"
assert 'W15: and nothing but stderr records it' "grep -q 'No such file' '$home/run.log'"
