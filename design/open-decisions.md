...
W15 and W17 closed 2026-09-03 (W15's layer-two half applied to the live env 2026-09-04); W16
closed the same day on its second pass, after review found the first one reachable-but-wrong.

**2026-09-04 — the epic closes here, and every row below now states which of three things it
is.** *Done on a branch, unmerged:* W9 (`fix/w9-asserts-join`), W11
(`test/w11-praetorium-status-suite`) and a new W18 (`fix/standing-research-task-file-guard`),
landed serially by hand after this wrap-up. Two of those three carry a defect adversarial
review found, named in the row rather than deferred out of it — an honest row is the product
here, not a clean one, and a row claiming a clean close that review disagreed with would be
worse than no row. *Decided, not built:* W8, W10, W13 and W14, each rewritten with an explicit
OWNER and an explicit TRIGGER that would reopen it. *Still genuinely open:* W12's reach, which
is a brief of its own and now says so with an owner and a trigger of its own; and W1's runtime
half, which is pending item 1 of Dave's list rather than carried work. Six months from now the
difference between "deliberately not done" and "forgotten" has to be readable off the row
itself, and a stated trigger is the only thing that carries it — a row that merely stops being
edited reads identically to one nobody remembered.

| | Item | Source | Note |
|---|---|---|---|
| W1 | Episodic memory keys on the OWNER persona, not the runtime profile name | D1 §6.2, §6.6 | **DONE — code 2026-09-02 (`cc9f802`), runtime applied 2026-09-04.** `AGENT_OWNER` now keys `MEM_DIR` while `AGENT_PROFILE` keeps keying cost.log's `profile=` — the two were one variable, which is why `~/.hermes/profiles/claude-opus/` was looked up and 77 runs logged `no-store`. **The row's old claim was half right and stays half right, for a different reason.** The fix reaches six jobs — `standing-research` (22 no-store runs), `raw-ingest` (22), `m1-signal-scan` (13), `bd-followup-drafts` (11), `weekly-pre-assembly` (5), `knowledge-digest` (4) — but `praetorium-daily-plan`, `praetorium-eod-summary`, `overnight-morning-report` and both campaigns run `AGENT_RUN_MODE=ops`, which skips memory by design (NUC-36, `agent_propose.sh:236,368-372`) and logs `na`, never `no-store`. Those four are **not** fixed by this and must not be: making ops mode record memory is a separate decision with its own evidence. The `AGENT_OWNER` lines from `docs/runbook.md` § W1 handoff are now in all nine live `~/.config/agent-workforce/*.env` files — verified 2026-09-04 by `grep -n`, one line per file, five `claudius` and four `marcus`, matching the handoff table exactly. **The first run that can prove it is later than the day it was applied**: `standing-research` still logged `memory=no-store` at 04:43 on 2026-09-04 because the lines went in after that run, and `no-store` fires only when `~/.hermes/profiles/$run_owner/memories` is absent (`bin/agent_propose.sh:408`). So a `no-store` row dated the same day as the fix is expected, not a failed handoff — read the NEXT run. |
...
| W8 | The six `praetorium-phaseb-brief@*` units have no cleanup owner | D8, brief 2 | **DECIDED 2026-09-04 — this is a deletion to sequence, not a defect to fix. OWNER: Dave (the `/etc` half needs `sudo`; the source half needs no privilege and no deploy). TRIGGER: none pending — the family is spent and deletable today; the trigger is the reverse, a decision to KEEP them past their expiry, which would need the `expires` date in `design/agents/trajan.toml` moved and a reason written beside it.** The rule stands as recorded: **delete from BOTH trees or neither.** Source and `/etc` are byte-identical, so nothing is red today; remove them from one tree alone and they become a membership difference. **The detection for that half-deletion is already armed, and this is verified rather than predicted.** Moving `systemd/praetorium-phaseb-brief@6.timer` out of source in a scratch worktree and re-running `bin/check_deploy_drift.sh` printed `DRIFT [system] etc-only: praetorium-phaseb-brief@6.timer — its campaign exclusion EXPIRED at 2026-09-02 05:22:00`, restored immediately; the `/etc`-side deletion is the symmetric case through the same code path. It fires because `design/agents/trajan.toml` declares the family `status = "campaign"` with `expires = "2026-09-02 05:22:00"` — already past — and because `campaign_expiry()` in the checker normalises an instance stem to its `praetorium-phaseb-brief@` family: key on the instance and a templated exclusion could never match at all. All five timers are spent (`list-timers --all`, last trigger 2026-09-02, no NEXT on any of them). **The precedent this row should point at arrived for real the same week, in the same mechanism.** D5's two campaign families fired their declared last runs on 2026-09-03 23:00 and 2026-09-04 01:30, their dated exclusions in `design/agents/augustus.toml` expired on exactly those timestamps, and `bin/check_deploy_drift.sh` now reports all four files as `DRIFT [system] etc-only: … its campaign exclusion EXPIRED at …` — the whole of today's drift red, and the only thing standing between `bin/verify.sh` and green (0 `FAIL:` lines). A dated exclusion going red the moment its subject became deletable is the mechanism working as designed, and it is the model for retiring this family: same terms, one difference — D5's units exist in `/etc` only, so their deletion is one tree, while phaseb exists in both and is two. Sequenced as pending items 7 and 8 below rather than left to a brief that does not exist. |
| W9 | `design/fleet-suites.toml`'s `asserts` list has no join to the assertions it names | D6, brief 1 | **DONE AND MERGED — `4139227` is in `main` (2026-09-04 07:23); the branch is gone. Both fail-opens named below were subsequently closed, re-verified 2026-09-07.** This row read "not merged yet" for three days after it merged, which is the failure mode of recording a branch name as state: a branch is deleted on merge, so the row's own evidence disappears and nothing recomputes the claim. Verify a W row by `git merge-base --is-ancestor <sha> main`, never by the branch existing. Both survivors are closed: the `asserts join` count is now compared against a figure derived in `tests/test_workflow_coverage.sh:64-72` from that file's own anchors (`join_reported_what_it_compared`), so deleting the rule no longer passes as a clean run; and `suite-paths-exist` now lives in that same suite (`:94`), which carries **no** box gate, so the missing-path case no longer inherits a box-only fail-open. What was true and remains true: What it was: the `asserts` strings and the suites they name were related by convention only — `test_workflow_coverage.py` read `path` and `owner` and never `asserts`, and `test_fleet_guards.sh` only checked the list was non-empty. **Measured before the change, and review reproduced the count to the digit from a clean checkout: 32 declared ids, of which 8 were anchored, 5 more appeared as `check <id>` calls, and 19 appeared nowhere at all in the files they named.** One of the two nine-id entries names a file containing no assertions whatsoever — an eight-line wrapper round a `.py`. The fix anchors each id in the suite that declares it, `::`-prefixed in parentheses on the line opening its group, and `asserts-anchored` reads the two sides against each other in **both** directions: a declared id nothing anchors is red, an anchor no entry declares is red. Post-change it is a clean bijection — 33 ids across 4 entries, 33 anchors — and the join follows the `exec python3` hop, so a suite whose assertions live in a companion anchors them there. The notation is not new: `design/agents/*.toml` already writes `test = "<suite>::<id>"`. Review mutation-tested past the revert and the reverse direction survives a weakened forward half (matching a declared id by substring instead of whole value is still caught). **Fail-open one: deleting the rule outright leaves the suite GREEN.** Replacing `tests/test_workflow_coverage.py` with main's copy while keeping the shell suite as committed exits 0 and prints `ok:` for a rule that no longer exists — the new `asserts join: N anchored id(s) …` line is printed and never asserted, in a file that already carries the precedent for asserting a printed count (`exempt_named` vs `exempt_counted` → `named_matches_counted`). One `assert` closes it. **Fail-open two, and it is the exact CI shape the placement rationale claims to avoid:** an entry whose `path` does not exist is silently `continue`d out of the join and the coverage suite stays green — renaming the ownership entry's path dropped 9 ids and 9 anchors, taking the unasserted count from 33 to 24, with `ok: every path named in a suite exists on disk` still printing. The compensating rule lives in `tests/test_fleet_guards.sh`, which is gated by `box_only_with` with an `exit 77` fallback at lines 38-39 and does not run off the box, so for the missing-path case the join inherits precisely the box-only fail-open cited as the reason for not living there. **One overstatement to hold onto:** the commit subject and the `fleet-suites.toml` header both say the list is joined "to the assertions it names", while what runs joins ids to anchor TEXT — deleting both `assert` lines under an anchored header leaves it green. The checker's own docstring states that limit correctly; the two prose sites read stronger than the mechanism. `design/eval-spec.md` is updated by the same branch and says the hazard is closed 2026-09-04; this row is the other half of that pair. |
| W10 | Two bespoke off-box predicates, neither able to use `box_only_with` | brief 2, brief 4 | **DECIDED — standing-declined, re-confirmed 2026-09-04. OWNER: whoever adds the third site, and nobody until then. TRIGGER: a third site appearing — two copies is a coincidence, three is a shape, and only three is enough evidence to design a helper that keeps the property instead of copying one that loses it.** `bin/check_deploy_drift.sh` and `tests/test_content_skill_extract.sh` each hand-roll a predicate meaning "this is not Praetorium", proven by the deployed runtime tree being absent too, because `box_only_with` means "skip if a path is absent" — the opposite of what both need, which is a **missing subject ON the box staying RED**. Declined at review 2026-09-02: a shared helper would have to span `bin/` ↔ `tests/`, and the naive consolidation costs the fail-closed property, which is the only property either site has. **Re-measured 2026-09-04: still exactly two, and the work done since argues for the decision rather than against it.** Brief 7 needed the same predicate for the newly adopted `buzz-team/` tree and did **not** add a third site — it added a *conjunct* to the existing one, so the predicate is now all three trees absent and the printed SKIP line gained a third path, updated in `tests/ci-expected-skips.txt` in the same commit. Adding a conjunct only makes an existing skip harder to reach, which is the safe direction; a third copy would not have been. The other `exit 77`s in `tests/` are not instances and should not be counted as the trigger: `tests/test_fleet_ownership.sh:48` and `tests/test_buzz_interactive_harness.sh:68` are `tomllib`-availability skips, and the five `box_only_with` callers are the fail-open case this predicate exists to avoid. |
| W11 | `bin/praetorium-status.sh` has no suite of its own | brief 5 | **DONE AND MERGED — `1cb2ded` is in `main` (2026-09-04 07:31); the branch is gone. The vacuous assertion was fixed, re-verified 2026-09-07: `tests/test_praetorium_status.sh:199` now carries `env -u BRAVE_API_KEY -u OPENROUTER_API_KEY -u LLM_BASE_URL -u XDG_RUNTIME_DIR`.** The defect, kept because it is the more useful half of this row: One file added, 610 lines; `bin/praetorium-status.sh` is byte-identical to `main` **by design** — the script is deployed, so editing it on a branch would make the drift check red — which means "revert the production change" here meant restoring the pre-brief-5 behaviour in the working tree. Review did exactly that, 16 mutations, 14 red. The headline: restoring the two-unit whitelist that `systemctl --user --failed` replaced reds 10 assertions, one of which names `ghost-unit.service`, a string that appears nowhere in `config/`, `design/`, `systemd/` or `bin/` — so the block provably asks the manager rather than a list, which is the regression nothing else on this box would notice. Partial loss is caught too, not just wholesale removal: `head -1` on the failed set, dropping only the second unit, reds exactly 1. Replacing the rc capture on the `--failed` assignment with an unconditional `true`, so the exit status is discarded, reds 4 and **only** the unreachable-bus group, so the failed-set and `none` branches are genuine controls rather than three restatements of one fact. **The defect, and it is this suite's own failure mode turned inward.** `every --user call carries XDG_RUNTIME_DIR` measures the reviewer's ambient environment, not the script: `run_status()` unsets `BRAVE_API_KEY`, `OPENROUTER_API_KEY` and `LLM_BASE_URL` but not `XDG_RUNTIME_DIR`, so the caller's value leaks into the stub. Deleting **all three** `XDG_RUNTIME_DIR=` assignments from the production script left the suite 92/92 green with that assertion printing `ok`; re-running the identical mutation under `env -u XDG_RUNTIME_DIR` reds that one assertion and no other. On this box, in the documented invocation, it certifies nothing while reading green — the same "a check that cannot fail is not a check" pattern the suite's own header rails against, one file over. One-line fix, not yet applied: add `-u XDG_RUNTIME_DIR` to the `env` invocation in `run_status()`. Two lesser notes recorded: `NF>=5` → `NF>=4` alone costs nothing (the kind filter still rejects a 4-column row), and the suite leaves ~21 `mktemp -d` roots per run with no trap — the majority convention here, but this box has a recorded `/tmp` inode-exhaustion incident from exactly that. **Deliberately left unregistered in `design/fleet-suites.toml`**, which is a flagged decision rather than an oversight: `praetorium-status.sh` is exec'd by no unit at all, so `no-orphan-suite` can never flag it (gate prints 21 of 52 unclaimed, 0 orphaned — green), and registering it would mean inventing a third `owner` value whose only enforcement lives in a suite that skips off the box. |
| W12 | `no-orphan-suite` is narrower than its name and cannot see most orphans | brief 6 | The assertion reads *every suite is claimed by a workflow or by `design/fleet-suites.toml`*, but `tests/test_workflow_coverage.py:188-198` only reports an unclaimed suite when its subject is exec'd **solely by an archived unit** (`dead` non-empty). An unclaimed suite whose subject is live is not reported at all. Measured by adding `tests/test_fleet_ownership.sh` in brief 6: it was unclaimed and the checker stayed silent — nothing would have flagged its absence, so registering it in `fleet-suites.toml` was deliberate rather than gate-driven. Same family as W9 and as the whitelist defect in `readiness-report-phantom-blockers`: a check that asks a narrow list whether it is fine cannot fail. Widening it would change what green means for the whole gate — every unclaimed suite would surface at once — so it is a brief of its own, not a line inside one. **UPDATE 2026-09-03:** the rule's one live finding is now resolved (see W6), so this row is purely about the rule's reach — nothing is red on it today, which is exactly the condition under which a narrow check is easiest to leave alone. Closing that finding also surfaced a second fail-open in the same join and it IS fixed: `tests/test_workflow_coverage.py:150` credits **any truthy** `owner`, so the value was effectively unconstrained. Harmless while every entry said `"fleet"` — one value cannot be mistyped into another — and not harmless the moment a second value existed, since `owner = "retiredtool"` reads as owned, silences the orphan rule for that suite, and looks correct. `OWNERS` in `tests/test_fleet_guards.sh` is now the single enforcing site, demonstrated red against exactly that typo, and `design/eval-spec.md` §6 points at it rather than restating the values — W9's hazard, avoided in the file W9 is about. **OWNER AND TRIGGER, added 2026-09-04 so this row is not the one implicit gap left in the epic.** It is the only W row that is neither done nor decided — deliberately, because widening the rule is a brief and this wrap-up is not one. OWNER: the brief that widens it, and nobody until that brief is written. TRIGGER: an unclaimed suite with a LIVE subject turning out to matter — the surfacing this rule cannot do is currently free of consequence because nothing is red on it, and W11 has just added a second deliberately-unclaimed suite on exactly that reasoning, which is the first evidence the reach is being relied on rather than merely tolerated. A third would be the point to stop tolerating it. |
| W13 | `verify-fleet.sh` and `check-loaded.sh` are sourced here but run nowhere in the gate | brief 7 | **DECIDED 2026-09-04 — the machine-level gate stays machine-level, and `~/CLAUDE.md` § Verification remains its owner. OWNER of the one residual question: the next brief that touches `buzz-team/`. TRIGGER: an edit to the adopted copy being reviewed as if the gate ran it — that is the specific mistake this decision makes possible, so it is the specific event that should reopen it.** Adoption made `buzz-team/verify-fleet.sh` reviewable and drift-checked; it did not make it run, and it should not. Its gates assert live `/proc` state, live relay membership and the deny-listed `~/.config/buzz-agents/` tree, so calling it from `bin/verify.sh` would make a PR red for BOX STATE rather than for the diff — the exact failure `tests/box_precondition.sh` exists to prevent one layer up. `check-loaded.sh` was not adopted at all and cannot be: it reads the deny-listed tree by design. **The residual question — whether a decidable-from-a-checkout subset should be split into a repo suite — is genuinely open, and measurement 2026-09-04 makes it narrower than it looks.** The script carries twelve gate families (`1/active` … `12/calibration`). The two whose *shape* is checkout-decidable are `11/roster`, a grep of `TEAM.md` for a `<PENDING-MINT>` placeholder, and `12/calibration`, a grep of the calibration pack for a `**version: N**` line — and both read files the adoption deliberately left on the box: `TEAM.md` and `aurelian-calibration.md` are declared box-only in `buzz-team/MANIFEST.toml`, and `bin/check_deploy_drift.sh` names them out loud every run (`box-only: TEAM.md — declared excluded`). So splitting out the decidable subset would first mean adopting its subjects, which is a separate decision with its own reasons. Meanwhile `tests/test_buzz_interactive_harness.sh` has taken the tree-shaped half already — manifest agreement in both directions, key-material absence, the mention-gated DAG, the route table, the kind checker and the drift manifest — plus exactly one gate-shaped assertion, that `verify-fleet.sh`'s identity table equals `bin/buzz_agents.env` in both directions. Nothing is broken today. |
| W14 | The `buzz-agent@*` units carry no `OnFailure=` | brief 7 | **DECIDED 2026-09-04 — a design question, not a missing line, and the 2026-09-04 measurement makes that sharper than this row previously stated. OWNER: Dave, because the fix restarts all five agents. TRIGGER: a dead-agent incident where the detection latency actually cost something, or a second compensating control that would make the hourly floor moot.** Every scheduled unit on this box alerts on failure; the five always-on ones do not — `systemd/user/buzz-agent@.service` has no `OnFailure=agent-alert@%n.service`. The asymmetry is the whole content: an always-on unit that exits **is** restarted by systemd, so an `OnFailure=` would alert on a crash loop, while the failure this box actually produces — a unit that stays up and stops answering — would never fire it. **Read off the live unit rather than inferred, the case is stronger than that.** `systemctl --user show buzz-agent@marcus.service` reports `Restart=on-failure`, `RestartUSec=5s`, `StartLimitIntervalUSec=10s`, `StartLimitBurst=5`. Consecutive restarts are at least 5s apart, so at most two or three starts fall inside the 10s window and the burst limit is never reached — the unit re-enters `active` and never `failed`. **An `OnFailure=` line added as-is would therefore fire approximately never, including on the crash loop it appears to cover.** Adding one without also changing the restart policy installs a control that reads as coverage and is not, which is the whitelist defect in a different costume — and a control that cannot fire is worse than a known gap, because the known gap is still on someone's list. `fleet-turn-check` is the real compensating control (hourly, +2min jitter, declared in `design/agents/trajan.toml` with `alerted = true`), so the detection floor for a dead agent is up to an hour against seconds for a timer job. That hour is the number to argue about; the missing line is not. |

| ~~W15~~ | ~~`m1-signal-scan` launches an EMPTY PROMPT when its task file is missing, and reports success~~ | W6's suites, 2026-09-03 | **DONE 2026-09-03 (`fix/w15-m1-empty-prompt-fail-open`) — but only layer one reaches the runtime on deploy.** What it was: `bin/run_m1_signal_scan_cc.sh:19` was `exec claude -p "$(cat "$TASK_FILE")"`, and because the failing `cat` sat in a command substitution used as an *argument* `set -e` never saw it — the agent launched with an empty prompt and the script exited 0. Layer two is why that was silent: the job set no `AGENT_VERIFY_CMD`, so `bin/agent_propose.sh:331` skipped the artifact check and the run landed on line 440 as `OK: run completed, agent produced no proposal` — `NOPROPOSAL`, exit 0, indistinguishable from a genuine "no signals at quality", verbatim the class the same file's line 278 comment says once cost 20 consecutive runs. **Layer one:** an explicit `[ -r "$TASK_FILE" ]` refusal before the substitution (`bin/run_m1_signal_scan_cc.sh:23`), the shape `bin/run_overnight_morning_report_cc.sh:30` has always had — `-r`, not `-f`, because a present-but-unreadable profile yields the same empty prompt. **Layer two is one change in two files, and splitting it would have broken a working job:** `bin/proposal_or_decline.sh` accepts a run only if it wrote this run's dated proposal *or* the log tail matches `^DECLINE:`, so `profiles/m1_signal_scan_cc_task.md` had to stop saying "print one line saying why" and start saying `DECLINE: <short reason>` **before** `AGENT_VERIFY_CMD` was wired — wiring it first turns every legitimate no-signal Monday and Wednesday into a hard FAIL. The wiring is in `profiles/m1_signal_scan.env.example` only: the live `~/.config/agent-workforce/m1_signal_scan.env` is a deny-listed credential path no branch may write, so layer two is inert until Dave installs it (pending item 6 below). The three characterization assertions in `tests/test_m1_signal_scan_smoke.sh` were rewritten, not deleted: they now assert the refusal (non-zero, path named on stderr, agent never launched), plus an unreadable-file case, plus a live round trip that runs the real `proposal_or_decline.sh` and proves the new sentinel passes it *and* the prose wording it replaced does not. |
| W16 | `bin/auto-sync` reports an untracked-only tree as "clean" and never syncs it | W6's suites, 2026-09-03 | **DONE 2026-09-03.** The gate is now `pending=$(git status --porcelain)` / `[ -z "$pending" ]`, which is the only view that agrees with the `git add -A` two lines below it. It was `git diff --quiet && git diff --cached --quiet`; **neither looks at untracked files**, so a tree holding only NEW files took the early return and printed `working tree clean. Nothing to do` every 15 minutes, indefinitely — the cost being not a lost edit but a delayed one with a wrong author story, the file waiting for some unrelated tracked change and then being swept in under an `Auto-sync:` subject dated days after it was written. `tests/test_auto_sync.sh`'s W16 group is rewritten to the opposite shape: the untracked-only tree is committed, reaches origin, carries the new file, and leaves the tree clean so the next tick is genuinely a no-op. **Exit 0 was never the tell** — the broken code exited 0 too — so the assertion that does the work is the negative one, `does NOT call an untracked-only tree clean`; reverting the fix reds 6 of the 7. **The widening is bounded, and that is asserted, not inferred:** `--porcelain` and `add -A` honour the same exclusions and neither lists ignored paths (`--porcelain` needs `--ignored`), so there is no class of work the new gate reports and the commit step cannot stage — which would have been a 15-minute empty-commit alert loop under `set -e`. A new `ignored_only` fixture pins it, alongside the clean case strengthened to check LOCAL history too, since an unpushed empty commit would leave origin untouched and hide exactly that regression. The status is captured on its own line rather than inline in the `if`, because `set -e` is suspended inside a condition and a git failure there would read as "clean". **SECOND PASS, same day — the first version of this fix was correct in principle and wrong against the tree it runs on, and adversarial review caught it before the merge rather than origin/main catching it after.** Two facts that were not in this row: `/etc/systemd/system/agent-workforce-auto-sync.service:36` is `ExecStart=/home/dave/dev/agent-workforce/bin/auto-sync` — the **SOURCE checkout**, so `bin/deploy` is not a gate on this going live and **merging alone arms it**; and that checkout held six agent worktrees under `.claude/worktrees/`, a path that was untracked, unignored and unexcluded. The old tracked-only gate never saw them. The new one does, and `git add -A` two lines later stages each as a **160000 gitlink** — an accidental submodule with no `.gitmodules`, recording a commit that exists only in this clone, pushed with exit 0 and no alert. The class was dismissed in the first pass on the grounds that the repo has no `.gitmodules`; that absence is precisely what makes it SILENT rather than the predicted fail-closed empty-commit loop, because git needs no `.gitmodules` entry to stage a nested repo. The suite missed it because `untracked_only` writes into `bin/`, a directory git already tracks — porcelain emits `?? bin/newly_added.sh` and `add -A` stages that one file, whereas an untracked DIRECTORY collapses to a single `?? nested/` entry whose expansion is a different set. Two entries, two behaviours, 33 green assertions. **Fixed on the property, not the path:** `bin/auto-sync` now scans the index after `add -A` and refuses on any gitlink with no `.gitmodules`, resetting the index first so the refusal is recoverable — non-zero reaches `OnFailure=agent-alert@%n.service`, which is the loud direction. `.gitignore` carries `.claude/worktrees/` as well, because an ignore list is what you remember and the guard is what you do not: ignoring the known path keeps the guard in reserve for the next nested repo nobody predicted. A `nested_repo` fixture and a ten-assertion group pin both halves, including the documented remedy (once ignored, the very next run syncs normally) and a positive control proving the origin-history scan can actually see a gitlink. Reverting only the guard reds 6 of the 10; the two that stay green are labelled in-place as companions rather than detectors. Verified against main: no gitlink exists anywhere in this repo's history and there is no `.gitmodules`, so the guard cannot trip on existing state. |
| ~~W17~~ | ~~The pre-snapshot's `Alerts (last 10)` has no freshness marker~~ | W6's suites, 2026-09-03 | **DONE 2026-09-03 (`fix/w17-alert-tail-freshness-marker`, PR #16).** The alerts section now renders the log's age beside the tail with a FRESH/STALE/UNKNOWN verdict, fail-soft so an unreadable mtime degrades to a stated unknown rather than a crash or a blank. **Two live sites, not one:** `profiles/overnight_morning_report_cc_task.md:76` reads the log DIRECTLY with its own `tail -20`, so fixing only the producer would have left the defect on the path Dave actually reads each morning; the consumer is now instructed to read the marker and say when the log is stale. The archived third site is deliberately untouched. Coverage 37->50 and 45->52; review mutation-tested past the revert (dating the run instead of the log costs 6 assertions, hardcoding STALE costs 3). **Two limits recorded rather than hidden:** the marker reads the FILE's mtime, so a touch or an in-place rotation would report freshness the log lacks — `bin/agent_alert.sh:67` appends today but nothing pins that — and it cannot separate "no failures recently" from "alerting is dead", since the log is only written on failure, which makes STALE the healthy steady state. **This row was still marked open on `main` after the PR merged**, and the summary line above it still read "W16-W17 stand"; corrected here from the W16 branch. Two sibling branches editing one table is how that happens — the second to merge carries a copy of the section cut before the first landed. |
| W18 | `bin/run_standing_research_cc.sh` reaches `$(cat "$TASK_FILE")` with no guard | W15's class, 2026-09-04 | **DONE AND MERGED — `4a06343` is in `main` (2026-09-04 07:16); the branch is gone. Review found no defect and none has since appeared.** Same shape as W15 one runner over: the substitution sits in an *argument*, so `set -e` never sees `cat`'s failure — it writes to stderr, yields the empty string, and the agent launches with an EMPTY PROMPT. The fix is one line — a `[ -r "$TASK_FILE" ]` test whose failure branch names the path on stderr and exits 1 — placed before the substitution **and** before `cd "$INBOX"`, with `-r` rather than `-f` because a present-but-unreadable profile feeds `cat` the identical failure. **Unlike W15 this is layer one only, and here that is the whole fix rather than a third of one.** m1 needed three parts because it wired no `AGENT_VERIFY_CMD` and its empty-prompt run logged as `NOPROPOSAL` exit 0; this job already carries both halves of layer two — `profiles/standing_research.env.example:33` wires `proposal_or_decline.sh standing-research` and `profiles/standing_research_cc_task.md:41` already carries the literal `DECLINE:` sentinel, both checked verbatim at those lines and both untouched by the diff — so an empty-prompt run here produces neither a dated proposal nor a decline and fails loudly. The guard buys the named path in the journal, not the alert. Suite 23 → 35 assertions with **zero** descriptions dropped (byte-sorted `LC_ALL=C comm` of the old and new sets). Review deleted only the guard line, confirmed by diff that the file content actually changed rather than trusting an index restore, and got 6 FAIL / exit 1; restoring returned 35 ok. Then eight shape-preserving mutations past the revert, all caught: `-r`→`-f`, guard moved below `cd "$INBOX"`, path hardcoded in the message instead of `$TASK_FILE`, `exit 1`→`exit 0`, wrong subject (`[ -r "$INBOX" ]`), always-true predicate (`[ -n "$TASK_FILE" ]`), message text changed, and the prompt replaced by a hardcoded string. **The scope claim was checked rather than accepted:** this was the last of the ten CC runners that actually read a `TASK_FILE` unguarded — 14 `run_*_cc.sh` exist, 4 are thin exec-shims into guarded runners, and all 10 that read one now carry the guard. Three limits recorded rather than hidden: the `chmod 000` fixture is uid-dependent and fails **closed** under root (as root the guard passes and the assertion goes red, so it can never certify falsely); the `both_missing` exit-code assertion is correct-by-coincidence and only its sibling grep discriminates ordering; and the guard writes to stderr while the harness merges both streams, so no assertion pins the stream. |
| W19 | The D5 campaign retirement left an executable chain behind | W17's class, 2026-09-04 | **OPEN — widened 2026-09-04; full write-up in `.claude/briefs/w19-campaign-retirement-residue.md`.** Opened as "two override envs outlive their units" (`~/.config/agent-workforce/content_strategy.env`, 16 lines, and `faceless_content.env`, 11). That was the visible tip. The retirement commit `1bc6a4c` touched exactly two files — `config/fleet-units.tsv` and `design/agents/augustus.toml` — **because those are the two the repo had a check for**: `tests/test_fleet_ownership.sh` failed in the manifest->list direction until the `.tsv` was re-materialised, and nothing else failed, so nothing else was touched. Eleven pieces of residue survive, and the five in `bin/` and `profiles/` form a closed, deployed, reachable chain (env -> `run_{content_strategy,faceless_content}_cc.sh` -> `run_standing_research_topic_cc.sh` -> the two task profiles), with `augustus.toml`'s `[surfaces.scheduled]` still `present = true` over zero workflows (`:33`) and its `augustus-content` note still describing a 01:30 lock collision with a unit the same commit deleted (`:78`). **`design/workflow-registry.md` is referenced by no test and no script** — verified by grep over `tests/` and `bin/` — so its rows `:77-78` still read `keep` with live triggers. That is the generalisable part and it is one level up from W17: a retirement is as complete as the repo's joins force it to be, and no more; a registry nobody joins against is prose, and prose does not get retired. The cleanup is **not piecemeal** — `bin/check_deploy_drift.sh:334` reports a runtime-only `bin/` file unconditionally while only the content loop consults `design/deploy-exclusions.toml` (`:392`), so deleting the three `bin/` scripts is hard red with no declarable exemption and needs `bin/deploy --prune`, which cannot be aimed and clears nine deferred exclusions. The `design/` and `profiles/` half can land alone and green. Items 1-2 stay Dave-only: the config path is deny-listed, so no check here can enumerate it and the only instrument is a human running `ls`. |
| W20 | Scheduled Buzz workflows cannot wake an agent on the installed harness | 2026-09-07, Dave-reported | **HARNESS HALF DONE 2026-09-07 — two halves remain, and neither is a binary.** `~/.local/bin/{buzz,buzz-acp}` upgraded 2026-07-31 build → **desktop-v0.5.23** (2026-09-05, extracted from `Buzz_0.5.23_amd64.deb`; upstream ships no standalone CLI asset). All three `buzz:workflow*` literals now present (were 0/0/0), five units restarted, `NRestarts=0`, `verify-fleet` and `check-loaded` both PASS. Joined by `tests/test_buzz_acp_update.sh` so it cannot silently regress (that assertion was `tests/test_buzz_acp_workflow_wake.sh` for a few hours the same day, then folded into the updater's `probe` so the install gate and the test gate cannot disagree about capability). **Staleness is now a watched condition rather than a thing nobody asks:** `agent-buzz-acp-update.timer` runs daily and alerts through `agent-alert@` when the box falls behind upstream, when the install receipt stops matching the bytes on disk, or when it has not reached GitHub for a week. It reports and stages; it does not install — `bin/buzz_acp_update.sh apply <tag>` is supervised, with a canary restart and automatic rollback, because a release that renames a flag crash-loops the fleet. **STILL OPEN: (a)** whether the deployed relay emits the tags — one authenticated `buzz workflows list`, which no session here can run; **(b)** the base prompt still advertises `buzz workflows` as `list, trigger, runs` — **unchanged at 0.5.23, verified against the new binary** — so agents still believe they cannot `create`, and asking one in chat still produces nothing. The upgrade removed the silent drop; it did not make scheduling work. Original finding: **OPEN — full write-up in `.claude/briefs/buzz-task-scheduling.md`.** Asking an agent in chat to schedule work fails two ways and both are silent. (1) The buzz-acp base prompt advertises `buzz workflows` as `list, trigger, runs` only, so an agent believes it cannot author one — while the installed CLI has `create`. Instruction-layer, not capability. (2) The decisive one: a workflow message is signed by the **relay**, and clearing `--respond-to owner-only` needs `verified_workflow_owner()` (`crates/buzz-acp/src/lib.rs:247` at upstream `3c7f288`) to re-attribute it via three `buzz:workflow*` tags. `~/.local/bin/buzz-acp` is dated 2026-07-31 and carries **zero** of those literals — `buzz:config-nudge` present as the extraction control — so the message is dropped with no error on either side, indistinguishable from a dead unit. Upstream's fix is #6953 (`CHANGELOG.md:124` under v0.5.22, `:246` under v0.5.21). **The first step is not the upgrade**: establish whether the deployed relay emits the tags at all, which takes one `buzz workflows list` from an agent's own shell. The HTTP probe run 2026-09-07 is a NON-TEST — 403 on the workflows route and 403 on a deliberately nonexistent path — and must not be cited as evidence either way. Scheduled agent-to-agent dispatch **already works** by another route (`augustus-content.timer` → `bin/run_content_via_buzz.sh`), because that message is signed by an agent credential rather than the relay's; the gap is that it is reachable only by editing this repo, never from chat. Upgrading restarts all five `buzz-agent@*` units and is Dave's, per § Pending actions. |
...
**Brought current 2026-09-04, and cleared items are struck IN PLACE rather than removed** —
for the same reason the paragraph above gives. Deleting items 1, 2 and 5 would renumber 6 into
3, and item 6 is referenced by number twice inside this file: from W15's row and from the
carried-work summary paragraph. **Checked rather than assumed, and the check moved the answer:**
nothing outside this file references any item by number — the seven external citations in
`profiles/`, `bin/` and `docs/` all name a *section* (`open-decisions.md D7`), which is the
shape that survives an insertion. So the renumbering hazard is real but internal, which is the
cheaper half. A cleared item that still occupies its slot also carries the more useful half of
the record: what closed it, and how that was verified. New work is appended, never inserted.

1. ~~**`bin/deploy`, after merge.**~~ **CLEARED for today's `main`, verified 2026-09-04 —
   and it REOPENS the moment this wrap-up's branches land.** `bin/check_deploy_drift.sh`
   reports no content drift and no source-only file in any of the six content trees;
   `diff -rq bin/ ~/agent-workforce/bin/` differs only by runtime debris the checker excludes
   by rule (seven `*.bak-*`, a `.bak`, one `__pycache__`). `bin/skill_sections.sh` is
   byte-identical at 3,442 B on both sides, so the "augustus reads the skill by the old pinned
   line ranges" half of this item is closed. **Do not read the clearance as a standing
   property.** W18 edits `bin/run_standing_research_cc.sh`, so merging it makes `bin/` drifted
   again and this item live again — which is exactly what the original text warned about: the
   number is a property of the tree at merge time, and every later branch moves it without
   noticing. Re-run the check; never quote a count from here.
2. ~~**`sudo` install of `agent-drift-check.{service,timer}`.**~~ **CLEARED — installed, and
   PROVEN BY RUNNING, verified 2026-09-04.** Both files are in `/etc/systemd/system/` (dated
   2026-09-03 11:49) and the drift check reports no membership difference for them in either
   direction. The timer is `active` / `enabled` with `LastTriggerUSec=Fri 2026-09-04 05:40:11
   CEST`; the service ran at that instant and exited `ExecMainStatus=1`, `Result=exit-code`.
   **That non-zero is the correct verdict, not a broken install** — it is the four expired D5
   campaign exclusions of item 7, i.e. the job saying the one true thing it had to say on the
   first morning it had anything to say. Item 3 is what made this checkable rather than
   assumed: `active` + `enabled` + a correct `next_elapse` would have reported all of the above
   for a unit whose `ExecStart` did not exist. The unit's `ExecStart` still names the SOURCE
   repo rather than the runtime tree, and that inversion is deliberate — see the unit header.
3. **Verify by RUNNING the job, not by `list-timers`.** `active` + `enabled` +
   a correct `next_elapse` say nothing about whether `ExecStart` exists. Not a task and never
   clears — it is the rule items 2 and 7 are checked against, and it earned its slot when two
   jobs sat green in `list-timers` while structurally incapable of producing anything.
...
5. ~~**`~/agent-workforce/bin/kanban_run_and_wait.sh` is still on disk**~~ **CLEARED — the
   ...
   check ignores by rule as `*.bak*` debris). It went **without** a `--prune`, and the record
   should say so plainly rather than credit a command nobody ran: `design/deploy-exclusions.toml`
   was authored on 2026-09-03 listing **nine** runtime-only files with no `bin/` entry among
   them, so the file was already absent when W7 measured the tree. **The block below is
   therefore stale in its premise and is corrected in place** — the one file anyone wanted
   removed is removed, and what `--prune` would delete today is twelve files nobody has asked
   for.
6. **Install `AGENT_VERIFY_CMD` into the live `~/.config/agent-workforce/m1_signal_scan.env`**
   (W15, layer two): `AGENT_VERIFY_CMD='~/agent-workforce/bin/proposal_or_decline.sh m1-signal-scan'`,
   copied from `profiles/m1_signal_scan.env.example`, then `chmod 600`. The branch fixed the
   template only — that tree holds credentials and is deny-listed to every agent here, and
   `bin/deploy` never writes it either, so **no deploy makes this line take effect**. Until it
   is installed, m1 keeps its empty-prompt guard (layer one, which does ship) but a run that
   dies for any other reason still logs as a clean `NOPROPOSAL`. Order matters and is already
   satisfied: `profiles/m1_signal_scan_cc_task.md` prints `DECLINE: <short reason>` as of this
   branch, so installing the line cannot fail a legitimate no-signal run — but if that profile
   change is ever reverted, revert this line with it. **Still pending as of 2026-09-04, and
   deliberately reported as UNVERIFIED rather than as pending.** `~/.config/agent-workforce/`
   is deny-listed to every agent on this box, so no branch, no wrap-up and no drift check can
   read whether the line is installed — the honest record is that this item's state is unknown
...
7. **Delete the two spent D5 campaign families from `/etc` — DUE NOW, and they are the whole
   ...
   2026-09-04 01:30:01, matching `expires` in `design/agents/augustus.toml` to the minute — and
   `list-timers --all` shows no NEXT for either; they are still `active` / `enabled`, which is
   the only reason they appear at all. They exist in `/etc` **only**, with no source copy, so
   this is a one-tree deletion, unlike item 8. `bin/check_deploy_drift.sh` reports all four as
   `DRIFT [system] etc-only: … its campaign exclusion EXPIRED at …`, and those four findings
   are the sole reason `bash bin/verify.sh` exits 1 today — the run carries **0** `FAIL:` lines.
   **One thing that belongs to this act and not to a follow-up:** once the files are gone, the
   two `[[workflows]]` campaign entries in `design/agents/augustus.toml` (lines 94 and 108) name
   units that exist in **no** tree, and *nothing checks that direction* — `timer-family-declared`
   joins `systemd/` → manifest and never manifest → tree, and `check_deploy_drift.sh` reads the
...
8. **Delete the `praetorium-phaseb-brief@` family from BOTH trees, in one act** (W8). Five
   ...
   check is correctly quiet, and the moment one tree loses them it goes red naming the expired
   campaign exclusion (demonstrated, source side, 2026-09-04). Ordered after item 7 only
   because item 7 is already red and this is not.

**`bin/deploy --prune` would delete twelve files, and as of 2026-09-04 not one of them is the
file anyone wanted gone.** That is a stronger version of the stop-and-report the brief asked
for, not a weaker one: the single wanted deletion was made by hand (item 5), so what `--prune`
now buys is entirely the eleven-plus-one nobody has decided on. The `bin/` row below is struck
because its subject is gone; the rest are unchanged and re-confirmed against
`check_deploy_drift.sh`'s nine `runtime-only` findings plus the three inert staging files.
Dry run, 2026-09-02:

| Tree | Prune would delete | What it is |
|---|---|---|
| ~~`bin/`~~ | ~~`kanban_run_and_wait.sh`~~ | **already deleted by hand, 2026-09-03 or earlier — this row is history** |
...
runtime (the twelfth is `discord-bot.service`, which is in no source tree at all), because
`bin/deploy` is additive: **`git mv` in this repo renames a file in source
...
~~**W7 is now measured rather than suspected.**~~ **FIXED 2026-09-03 — this paragraph is kept
as the record of what the gap was, not as a live finding.** It read: `bin/deploy` ships eight
paths, `check_deploy_drift.sh` compares two (`bin/`, the unit trees), and `profiles/`, `docs/`,
`config/`, `CLAUDE.md`, `AGENTS.md` and `README.md` drift unwatched — which is how five archived
profiles sat live in the runtime for a day with a green drift check. All eight are compared as
of W7, and those same five archived profiles are now the visible half of the `--prune` backlog
above, declared in `design/deploy-exclusions.toml` and named in every run. **A paragraph that
was true when written and is false when read is the defect this epic keeps finding**, so it is
struck rather than deleted: deleting it loses the evidence that the coverage gap was real.
...
9. **Establish the relay's workflow support** (W20). **The binary upgrade — formerly step two of this item — was done 2026-09-07 and is no longer yours**; see W20. What is left is genuinely yours, and it is smaller. Original framing kept because the ordering lesson outlived it: two
   steps, and the first one is not the upgrade. **Step one costs one command and settles the whole
   question**: `buzz workflows list --channel <uuid>` from any agent's own shell — no Claude Code
   session here can run it, because `~/.config/buzz-agents/` is deny-listed and holds the only
   credentials on this box. The HTTP probe already run is a **non-test** (403 on the workflows route
   *and* on a deliberately nonexistent path) and must not be reported as a result either way.
   ~~**Step two, only if the relay does support it**: upgrade the binaries.~~ **DONE 2026-09-07,
   and deliberately ahead of step one.** Deferring it was the wrong call: the artifact already
   existed, the swap is two `mv`s plus five restarts, the old pair is kept at
   `~/.local/bin/buzz-backup-2026-09-07/`, and the binaries were five weeks stale on their own
   merits regardless of the relay. **A cheap step that is necessary either way should not wait
   behind a step blocked on someone else.**
   ~~**Step two is now the authority decision**: granting `create` hands five agents the ability
   to schedule recurring work that runs with your standing authority.~~ **GRANTED 2026-09-07 on
   Dave's instruction.** `~/.config/buzz-team/TEAM.md` now carries a `## Scheduling work` section
   that overrides the 0.5.23 base prompt's `list, trigger, runs`; it requires an agent to run
   `buzz workflows list` and report the result before promising anything, forbids `call_webhook`
   (an outward HTTP call, which is the one hard gate in the charter), states plainly that a
   workflow runs with Dave's authority rather than the agent's, and requires announcing and
   deleting what it creates. The pin is bumped in `buzz-team/MANIFEST.toml`.
   **So step one is the only thing left in this item**, and note what the grant did to it: the
   first agent asked to schedule anything now runs `buzz workflows list` as its opening move and
   reports the answer. Step one stopped being a task and became a thing that happens.
   Brief: `.claude/briefs/buzz-task-scheduling.md`.
...
6. **W1–W4**, then **D5's cleanup**: delete the two campaign `/etc` units after their last
   firings (content-strategy 09-03 23:00, faceless 09-04 01:30). **Both firings have happened
   and the deletion is now due** — it is pending item 7, and until it runs it is the only red
   in `bin/verify.sh`.

**D4, D2 and D5 need no brief** — D4 and D2 were resolved as decisions, and D5 was a "no".

**The carried-work list was closed as of 2026-09-04, and reopened once since.** Every W row is
done, merged, or a decision with an owner and a trigger — including W12, whose open question
carries both rather than sitting in the list unattributed. **W20 was added 2026-09-07** (Buzz
workflow wake, Dave-reported) and is the one open item alongside W19. Nothing above waits on a
brief that does not exist.

**"Done on a named branch" is no longer a state, and was never a durable one.** W9, W11 and W18
each recorded a branch name as their evidence; all three merged on 2026-09-04 and the branches
were deleted, so for three days the list described parked work that had landed — the row's own
evidence had been garbage-collected out from under it. Record the **commit sha**, which survives
the branch, and check a row with `git merge-base --is-ancestor <sha> main` rather than by looking
for the branch. What remains is the eight pending items, which name Dave, and the two standing
side items below, which still name nobody — recorded here as the one list in this file that
has no owner, which is the honest state rather than a tidy one.
