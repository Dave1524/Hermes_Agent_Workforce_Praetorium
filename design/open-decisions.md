...
| W19 | Two retired campaigns' override envs outlive their units | W17's class, 2026-09-04 | **OPEN — Dave's action, and the drift check cannot see it.** `~/.config/agent-workforce/content_strategy.env` (16 lines) and `faceless_content.env` (11) are still on disk, but `praetorium-{content-strategy,faceless-content}-research.{service,timer}` were deleted from `/etc` on 2026-09-04 (item 7) and exist in no tree. Nothing reads either file. **This is W17's defect pointing the other way, one tree over:** W17 was a deploy destination compared by nothing, and `~/.config/agent-workforce/` is a destination compared by nothing *and* unreadable to every agent on this box — it is on the Claude Code deny-list, so no check here can enumerate it, and `bin/deploy` never writes it. That is why this row exists rather than an assertion: the only instrument is a human running `ls`. Deleting them is safe (no unit names either path) but is not urgent — a config file for a unit that cannot start is inert, not dangerous. Found by reading a `wc -l` Dave pasted for an unrelated check, which is the *only* way it could have been found. |
