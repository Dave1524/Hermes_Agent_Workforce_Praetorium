...
9. **Establish the relay's workflow support, then decide the Buzz binary upgrade** (W20). Two
   ...
   **Step two, only if the relay does support it**: `~/.local/bin/buzz` and `~/.local/bin/buzz-acp`
   are both dated 2026-07-31 and predate #6953, so no scheduled workflow can wake an agent here
   regardless of what the relay does. Upgrading restarts all five `buzz-agent@*` units and takes the
   live chat surface down — a fleet operation, not a repo change. Prove the fix with the same
   `strings` check that found the defect: the new binary must carry `buzz:workflow`,
   `buzz:workflow-owner` and `buzz:workflow-mention`, with `buzz:config-nudge` as the extraction
   control. Brief: `.claude/briefs/buzz-task-scheduling.md`.
