...
9. **Establish the relay's workflow support** (W20). **The binary upgrade — formerly step two of this item — was done 2026-09-07 and is no longer yours**; see W20. What is left is genuinely yours, and it is smaller. Original framing kept because the ordering lesson outlived it: two
   ...
   **Step two is now the authority decision, and it is a real one:** the 0.5.23 base prompt still
   tells every agent `buzz workflows` means `list, trigger, runs`. Granting `create` is one line in
   the team-instruction layer (`~/.config/buzz-team/TEAM.md`, which layers after `[Base]`), but it
   hands five agents the ability to schedule recurring work that runs with your standing authority
   — cheap to type, not cheap in consequence. Brief: `.claude/briefs/buzz-task-scheduling.md`.
