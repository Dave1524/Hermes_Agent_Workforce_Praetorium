...
`main`, every fifteen minutes** (`bin/auto-sync:86`, `git add -A`). That is its purpose and its
...
- **Verdict line** — one journal line per run, `auto-sync: working tree clean. Nothing to do.`
  (`:79`) or `auto-sync: pushed main at <short sha>` (`:117`); off `main` inside the grace
  window, `auto-sync: not on main (current: <branch>) for <h>h<mm>m (since <UTC>), inside the
  <n>h grace window. Skipping; nothing committed.` and exit 0 (`:51-52`). The refusals (`not on
  ...
  stderr and exit 1 (`:36-49`, `:56-58`, `:104-110`).
...
- **Next action:** on `not on main` past the window, finish or park the branch and check out
  ...
  nested repo to `.git/info/exclude` (`:107-109`).
...
the verdict line above and exits 0 (`:29-53`). A session that switched this checkout to a
...
a failure by design, so that the alert says *which* repo state needs a human (`:36-49`,
`:56-58`, `:104-110`) — including "cannot tell since when", which is loud on purpose.

## Side effects

- Commits with `git add -A` — honours `.gitignore` and `.git/info/exclude`, nothing else
  (`:70-72`). A file the deny-list withholds from agents is not withheld from this job; it is
...
- **New-only trees looked clean until 2026-09-03.** `git diff` misses untracked files;
  `git status --porcelain` is now the test (`:63-80`, W16). Do not "simplify" it back.
