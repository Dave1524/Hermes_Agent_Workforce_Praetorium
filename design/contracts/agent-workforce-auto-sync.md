...
`main`, every fifteen minutes** (`bin/auto-sync:88`, `git add -A`). That is its purpose and its
...
syncs no branch but `main` — off it, it skips quietly for a grace window and then refuses
(`:29-55`).
...
| Remediation owner | Dave — every failure path is a repo state only a human should resolve: a branch left checked out past the grace window, non-fast-forward, staged gitlinks |
| Retirement condition | none — standing; ends when the source repo stops being edited on this box |
| Contract version | 2 (2026-09-23, off-`main` grace window — a fresh branch no longer alerts) |
...
- **Verdict line** — one journal line per run, `auto-sync: working tree clean. Nothing to do.`
  (`:81`) or `auto-sync: pushed main at <short sha>` (`:119`); off `main` inside the grace
  window, `auto-sync: not on main (current: <branch>) for <h>h<mm>m (since <UTC>), inside the
  <n>h grace window. Skipping; nothing committed.` and exit 0 (`:53-54`). The refusals (`not on
  main` past the window, with no reflog entry to date the departure, or with a malformed window;
  `could not fast-forward`; `REFUSING to sync — nested repositories staged as gitlinks`) go to
  stderr and exit 1 (`:36-51`, `:58-60`, `:106-112`).
...
- **Next action:** on `not on main` past the window, finish or park the branch and check out
  `main` (the refusal is correct, not a fault — and branch work belongs in a worktree, which
  never moves this checkout); on `could not fast-forward`, rebase by hand; on gitlinks, add the
  nested repo to `.git/info/exclude` (`:109-111`).
...
Off `main` for less than `AUTO_SYNC_OFF_MAIN_GRACE_HOURS` (default 4; `0` never skips) since
the newest `checkout: moving from main` entry in the HEAD reflog: the run syncs nothing, logs
the verdict line above and exits 0 (`:29-55`). A session that switched this checkout to a
feature branch is a fault only once it is forgotten; paging on every tick of the first hours
sent 7 alerts on 2026-09-18 alone. Everything else that stops the sync is a refusal (exit 1),
a failure by design, so that the alert says *which* repo state needs a human (`:36-51`,
`:58-60`, `:106-112`) — including "cannot tell since when", which is loud on purpose.

## Side effects

- Commits with `git add -A` — honours `.gitignore` and `.git/info/exclude`, nothing else
  (`:72-74`). A file the deny-list withholds from agents is not withheld from this job; it is
...
- **New-only trees looked clean until 2026-09-03.** `git diff` misses untracked files;
  `git status --porcelain` is now the test (`:65-82`, W16). Do not "simplify" it back.
...
- **The grace clock restarts on every return to `main`.** It measures time since the newest
  departure, so a checkout that visits `main` and leaves again gets a fresh window. A branch
  genuinely forgotten stops moving, and then alerts.
- **Check 2 still fails inside the grace window.** It asks whether the checkout is on `main`,
  and within the window it is not. That is a sweep receipt, not a page: the incidents route
  carries `notify=none`.
