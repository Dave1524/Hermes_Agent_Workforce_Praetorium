...
logical id, with `retry` taken from the contract's Identity `Retry` row. The bytes are
deterministic (sorted keys, no timestamp) so the drift check can compare them.
...
def _exclusion_reason(repo: pathlib.Path, entry: dict[str, Any], logical: str, scope: str) -> str | None:
    ...
    if kind != "timer":
        return f"kind = {kind} (always-on, not a timer workflow)" if kind == "service" else f"kind = {kind}"
...
def render(repo: pathlib.Path | str) -> dict[str, Any]:
    ...
    return {"schema": 1, "workflows": workflows, "excluded": sorted(excluded, key=lambda row: row["unit"])}
