...
## Decisions 2026-09-23 (Dave)
- **One PR**, branch `feat/t8-5-claude-md-one-page`, one commit per cluster in the order A, C, B, E, D, F.
- **Row 23: assert + fix all three** — `systemd/qmd-mcp.service:5`, `systemd/qmd-refresh.service:16`, `~/.config/qmd/index.yml:8` resolve `~/vault` via `readlink -f`; the `qmd-mcp` restart is rail 6 (no override) → Dave's hand, command handed over at land.
- **Existing-test edits run under `SHIP_RAILS_OVERRIDE="T8.5 trap-to-test"`** (T8.1 archive :127 — intended, permanent). Each is logged in `.git/ship_rails_overrides.log`; the PR body lists them.
