...
> ## STATE — 2026-09-04: BOTH HALVES DONE. Archived.
>
> **W-half done and merged.** Criteria 1-11 and 14-15 landed on `feat/phaseb-implementation`
> (`1713902..a27d577`) and `a27d577` is an ancestor of `origin/main`. Verified 2026-09-04:
> `bin/agent_propose.sh:251` keys `MEM_DIR` on `$run_owner`, and
> `tests/test_fleet_ownership.sh` exists and is registered at `design/fleet-suites.toml:97`.
> W1-W4 are marked DONE at `design/open-decisions.md:1055-1058`.
>
> **D5 half done — the gate this brief was held open for has passed.** Both campaigns fired
> their last declared runs (2026-09-03 23:00 and 2026-09-04 01:30, matching `expires` to the
> minute), and criteria 12-13 landed in PR #22 (`1bc6a4c`, 2026-09-04 13:05). Verified: the
> four `/etc/systemd/system/praetorium-{content-strategy,faceless-content}-research.*` files
> are gone, and `design/agents/augustus.toml:93-100` carries the removal note D5 item 3
> required. The two `[[workflows]]` entries were **removed rather than marked `spent`** —
> `spent` is for an entry whose unit still has source (`in_repo = true`, as the
> `nekovri-subsidy-*` entries do); these had none, so a surviving entry would name a unit
> that exists in no tree. That is a correction to this brief's criterion 13, which specified
> `"spent"`; `design/open-decisions.md` item 7 carries the reasoning.
>
> **Criterion 16 is now met on `main`, though never on this branch.** The two W6 blockers it
> reported are closed (`design/open-decisions.md:1060`, DONE 2026-09-03, both halves), and
> the drift was the documented feature-branch condition. Measured on `f738ff7` 2026-09-04:
> `bin/verify.sh` exits 0, `drift: clean`, 2558 assertions, 0 FAIL, one expected skip.
>
> **TWO HUMAN HANDOFFS REMAIN OPEN, and archiving this file does not close them.** Both are
> `~/.config/agent-workforce/*.env` edits — deny-listed, so no agent on this box can read,
> write or verify them — and both are recorded outside this brief, which is what makes it
> safe to archive:
>
>   - **W1, layer two.** The `AGENT_OWNER` lines. Until they are added, the six affected jobs
>     keep logging `memory=no-store` and the code fix reaches nothing on the box. Recorded at
>     `design/open-decisions.md:1055` ("RUNTIME PENDING DAVE") and spelled out with the exact
>     key and value per job at `docs/runbook.md:44` § W1 handoff.
>   - **W15, layer two.** `AGENT_VERIFY_CMD` for `m1-signal-scan`, one line appended to
>     `~/.config/agent-workforce/m1_signal_scan.env`; the value is already in
>     `profiles/m1_signal_scan.env.example:37`. Recorded at `design/open-decisions.md:1036`
>     ("W15's layer-two half waits on Dave, pending item 6").
>
> The banner this replaced read "W-half DONE, D5 half BLOCKED" and stayed accurate for two
> days after it stopped being true — the blocker was a date, and nothing re-reads a date.
